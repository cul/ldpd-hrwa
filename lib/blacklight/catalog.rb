...
module Blacklight::Catalog
  unloadable
  #TODO: Remove unloadable line (above)
  ...
  included do
    ...
    def show
      ...
      if(params[:last_known_s_search_json_string].nil?)
        session[:search][:results_view] = false
      else
        last_known_s_search_hash = ActiveSupport::JSON.decode(params[:last_known_s_search_json_string])
        last_known_s_search_hash['counter'] = params[:counter]
        last_known_s_search_hash['results_view'] = true
        last_known_s_search_hash.symbolize_keys!
        session[:search] = last_known_s_search_hash
    ...
    def update
      ...
      # HRWA Note: Call show action manually, don't redirect to it.  Then manually render show view. We want to preserve the post variables.
      show()
      render 'show'
    end
