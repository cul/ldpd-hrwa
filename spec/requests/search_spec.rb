...
#get the current host
  ...
  it 'use correct host when building URLs for facets: ' + current_host + ':3020' do

    #page_to_visit = 'http://' + current_host + ':3020' + '/search?type=archive&search_mode=advanced&q_and=women&q_phrase=&q_or=&q_exclude=&lim_domain=&lim_mimetype=&lim_language=&lim_geographic_focus=&lim_organization_based_in=&lim_organization_type=&lim_creator_name=&crawl_start_date=&crawl_end_date=&rows=10&sort=score+desc&path=%2Fsolr-4%2Fasf&submit_search=Advanced+Search'
    #visit page_to_visit

    # # page.should # NOT see SOLR host, should see Web host
    #page.find_link('English')[:href].should match('^/catalog')
